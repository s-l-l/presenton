...
export default function DoubaoConfig({
  doubaoApiKey,
  doubaoModel,
  webGrounding,
  disableThinking,
  ...
  const [apiKey, setApiKey] = useState(doubaoApiKey);
  const isImageGenerationDisabled = llmConfig?.DISABLE_IMAGE_GENERATION ?? false;

  const doubaoUrl = "https://ark.cn-beijing.volces.com/api/v3";
  ...
  const onApiKeyChange = (value: string) => {
    setApiKey(value);
    onInputChange(value, "doubao_api_key");
  };

  const fetchAvailableModels = async () => {
    if (!doubaoApiKey) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.length > 0) {
            onInputChange(data[0], "doubao_model");
        }
      } else {
  ...
  return (
    <div className="space-y-6 ">
      ...
      <div className="mb-4 flex items-center justify-between bg-white p-10">
        <div className="">
          <h3 className="text-xl font-normal text-[#191919]">Doubao API key</h3>
          ...
          <div className="relative  w-[275px] ">
            ...
            {(!modelsChecked || (modelsChecked && availableModels.length === 0)) && (
              <button
                ...
                disabled={modelsLoading || !doubaoApiKey}
                className={` mt-7 py-2.5 bg-[#F7F6F9] px-3.5 rounded-[48px] text-xs font-semibold text-[#101323] transition-all duration-200 border ${modelsLoading || !doubaoApiKey
            ...
            {modelsChecked && availableModels.length === 0 && (
              <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-sm text-yellow-800">
                  No models found. Please make sure your API key is valid and has access to Doubao models.
                </p>
              </div>
            )}
                      ...
                      >
                        <div className="flex gap-3 items-center">
                          <span className="text-sm font-medium text-gray-900">
                            {doubaoModel
                              ? availableModels.find(model => model === doubaoModel) || doubaoModel
                              : "Select a model"}
                          </span>
                        </div>
                          ...
                          <CommandGroup>
                            {availableModels.map((model, index) => (
                              <CommandItem
                                ...
                                value={model}
                                onSelect={(value) => {
                                  onInputChange(value, "doubao_model");
